...
export default function Select({ label, helperText, error, options = [], className = '', ...props }) {
  return (
    <div className={`flex flex-col mb-4 ${className}`}>
      ...
      >
        {options.map((opt, i) => (
          <option key={i} value={opt.value} disabled={opt.disabled} hidden={opt.hidden}>{opt.label}</option>
        ))}
      </select>
      {helperText && (
